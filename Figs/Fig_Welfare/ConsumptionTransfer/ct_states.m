...
min_ct_demand = min(CT_demand);
inx_demand = find(min_ct_demand == CT_demand);
...
min_ct_sun = min(CT_sun);
inx_sun = find(min_ct_sun == CT_sun);
...
min_ct_sun_demand = min(CT_demand_sun);
inx_sun_demand = find(min_ct_sun_demand == CT_demand_sun);
...
%     plot(400*(cPItarg-1),X(:,:,j),colors{1},'LineWidth',2);
    if j == 1
        ...
        set(gca,'XLim',[cPItarg_min_ann cPItarg_max_ann],'XTick',[cPItarg_min_ann, 0, 2, 4],'YLim',[0 0.2],'FontSize',15)
    ...
    elseif j == 2
        ...
        set(gca,'XLim',[cPItarg_min_ann cPItarg_max_ann],'XTick',[0, 2, 4],'YLim',[0 2],'FontSize',15)
        line([400*(cPItarg(inx(j))-1) 400*(cPItarg(inx(j))-1)],get(gca,'YLim'),'Color','b','LineStyle','-','LineWidth',1);
    elseif j ==3
        cPItarg_min_ann = min_inf_targ_sun_demand; 
        cPItarg_max_ann = 4;
