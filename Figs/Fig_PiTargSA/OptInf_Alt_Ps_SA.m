...
% cd /mq/philipprojects/RA_Work/Taisuke_Nakata/Zero_Lower_Bound/DeflationaryRegime/OptInf/draft/Figs/SA
% matlab -nodesktop -nosplash -r OptInf_Alt_Ps_SA
%--------------------------------------------------------------------------
...
%                             msg  = sprintf('p_c = %.2f, p_z = %.4f,  pi_targ = %.1f.',cases(kk,1),cases(kk,2),cases(kk,3));
                            msg  = sprintf('p_c = %.2f, p_z = %.4f,  pi_targ = %.1f, p_d =%.3f.',cases(kk,2),cases(kk,3),cases(kk,4),cases(kk,1));
                            disp(msg);
...
%% Plotting                
opt_inf_unc_plot = [opt_inf_unc(:,1,7),opt_inf_unc(:,1,8),opt_inf_unc(:,1,9)...
                    opt_inf_unc(:,1,9),opt_inf_unc(:,1,1),opt_inf_unc(:,1,2)...
                    opt_inf_unc(:,1,3),opt_inf_unc(:,1,4),opt_inf_unc(:,1,9)...
                    opt_inf_unc(:,1,5),opt_inf_unc(:,1,6),opt_inf_unc(:,1,9)];                
...
pd_string = {'p_D = 0.95','p_D = 0.965','p_D = 0.975'};
...
hold on
for i = 1:3
    h(i) = plot(p_s,opt_inf_unc_plot(:,i),colors_pd{i},'LineWidth',2);
    xlabel('p_T','FontSize',15)
    ylabel('Optimal Inflation Target','FontSize',15)
    set(gca,'XLim',[0.99 1],'XTick',(0.99:0.002:1),'Ylim',[min(min(opt_inf_unc_plot(:,1:3))) 2],'FontSize',15)
end
L = legend([h(1) h(2) h(3)],pd_string{1},pd_string{2},pd_string{3},'Location', 'NorthWest');
set(L,'FontSize',15)
...
hold on
for i = 4:6
    hh(i) = plot(p_s,opt_inf_unc_plot(:,i),colors_pc{i-3},'LineWidth',2);
    xlabel('p_T','FontSize',15)
    ylabel('Optimal Inflation Target','FontSize',15)
    set(gca,'XLim',[0.99 1],'XTick',(0.99:0.002:1),'Ylim',[min(min(opt_inf_unc_plot(:,1:3))) 2],'FontSize',15)
end
LL = legend([hh(4) hh(5) hh(6)],pc_string{1},pc_string{2},pc_string{3},'Location', 'NorthWest');
set(LL,'FontSize',15)
...
hold on
for i = 7:9
    hhh(i) = plot(p_s,opt_inf_unc_plot(:,i),colors_pz{i-6},'LineWidth',2);
    xlabel('p_T','FontSize',15)
    ylabel('Optimal Inflation Target','FontSize',15)
    set(gca,'XLim',[0.99 1],'XTick',(0.99:0.002:1),'Ylim',[min(min(opt_inf_unc_plot(:,4:6))) 2],'FontSize',15)
    
end
LLL = legend([hhh(7) hhh(8) hhh(9)],pz_string{1},pz_string{2},pz_string{3},'Location', 'NorthWest');
set(LLL,'FontSize',15)
...
hold on
for i = 10:12
    hhhh(i) = plot(p_s,opt_inf_unc_plot(:,i),colors_pitarg{i-9},'LineWidth',2);
    xlabel('p_T','FontSize',15)
    ylabel('Optimal Inflation Target','FontSize',15)
    set(gca,'XLim',[0.99 1],'XTick',(0.99:0.002:1),'Ylim',[min(min(opt_inf_unc_plot(:,7:9))) 2],'FontSize',15)
    
end
LLLL = legend([hhhh(10) hhhh(11) hhhh(12)],pitarg_string{1},pitarg_string{2},pitarg_string{3},'Location', 'NorthWest');
set(LLLL,'FontSize',15)
