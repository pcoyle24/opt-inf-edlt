function residuals_sun = sun_solve_2(x_guess_sun,i,j)
...
residuals_sun(2) = Ys/Cs^(cCHIc)*(cVARPHI*(PIs/((cPItarg(j)^cIOTA*PIs^(1-cIOTA))^cALPHA(i)) - 1)*PIs/((cPItarg(j)^cIOTA*PIs^(1-cIOTA))^cALPHA(i)) - (1 - cTHETA) - cTHETA*(1-cTAU)*Ws) - ...
    (cBET*cDELz*cVARPHI*(p_s*((Ys/Cs^(cCHIc))*(PIs/((cPItarg(j)^cIOTA*PIs^(1-cIOTA))^cALPHA(i)) - 1)*PIs/((cPItarg(j)^cIOTA*PIs^(1-cIOTA))^cALPHA(i))) + (1-p_s)*((Yd/Cd^(cCHIc))*(PId/((cPItarg(j)^cIOTA*PId^(1-cIOTA))^cALPHA(i)) - 1)*PId/((cPItarg(j)^cIOTA*PId^(1-cIOTA))^cALPHA(i)))));
...
residuals_sun(7) = Yd/Cd^(cCHIc)*(cVARPHI*(PId/((cPItarg(j)^cIOTA*PId^(1-cIOTA))^cALPHA(i)) - 1)*PId/((cPItarg(j)^cIOTA*PId^(1-cIOTA))^cALPHA(i)) - (1 - cTHETA) - cTHETA*(1-cTAU)*Wd) - ...
    (cBET*cDELz*cVARPHI*(p_d*((Yd/Cd^(cCHIc))*(PId/((cPItarg(j)^cIOTA*PId^(1-cIOTA))^cALPHA(i)) - 1)*PId/((cPItarg(j)^cIOTA*PId^(1-cIOTA))^cALPHA(i))) + (1-p_d)*((Ys/Cs^(cCHIc))*(PIs/((cPItarg(j)^cIOTA*PIs^(1-cIOTA))^cALPHA(i)) - 1)*PIs/((cPItarg(j)^cIOTA*PIs^(1-cIOTA))^cALPHA(i)))));
