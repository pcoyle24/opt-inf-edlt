%--------------------------------------------------------------------------
%File Name: IRFs_sun_shock_neg.m
%Author: Philip Coyle
%Date Created: 02/02/2018
% cd /mq/philipprojects/RA_Work/Taisuke_Nakata/Zero_Lower_Bound/DeflationaryRegime/draft/Figs/Fig_4
% IRFs_sun_shock_neg.m
%--------------------------------------------------------------------------
...
cPItarg_min_ann = -0.99; 
cPItarg_mid_ann = 0; 
cPItarg_max_ann = 2;
cPItarg_min = cPItarg_min_ann/400 + 1;
cPItarg_mid = cPItarg_mid_ann/400 + 1;
cPItarg_max = cPItarg_max_ann/400 + 1;

cPItarg     = [cPItarg_min cPItarg_mid cPItarg_max]; 
p_s         = 0.995; 
p_d         = 0.975; 
cDELz       = 1;
c           = params(2);
...
pi_targ_string = {'-0.99% \Pi^{targ} (R)','0% \Pi^{targ} (L)', '2% \Pi^{targ} (L)'};
colors = {'k','b','r'};
...
for i = 1:length(cPItarg)
    for j = 1:length(header)
        ...
        if j == 2
            ...
            if i ~= 1
                set(gca,'YLim',[-3 3],'FontSize',15)
            else
                set(gca,'YLim',[-9e-3 -1e-3],'FontSize',15)
            end
        else
...
%             ylabel(y_label{j},'FontSize',15)
            title(header{j},'FontSize',10,'FontWeight','normal')
        end        
    end
end
L = legend([h(1) h(2) h(3)],pi_targ_string{1},pi_targ_string{2},pi_targ_string{3},'Location', 'North');

set(fig(1),'PaperOrientation','Landscape');
set(fig(1),'PaperPosition',[0 0 11 8.5]);
print(fig(1),'-depsc','sunspot_only_IRFs_neg.eps');