%--------------------------------------------------------------------------
%File Name: IRFs_sun_shock_2by2.m
%Author: Philip Coyle
%Date Created: 02/02/2018
% cd /mq/philiprojects/RA_Work/Taisuke_Nakata/Zero_Lower_Bound/DeflationaryRegime/OptInf/draft/Figs/Fig_4
% IRFs_sun_shock_2by2
%--------------------------------------------------------------------------
...
for i = 1:2
    Xs(:,i,1) = 400*(PIs(i)-1);
%     Xs(:,i,2) = 100*(Ys(i)-1);
    Xs(:,i,2) = 100*(cVARPHI/2*(PIs(i)/cPItarg(i)-1)^2*Ys(i));
    Xs(:,i,3) = 100*(Cs(i)-1);
    Xs(:,i,4) = 400*(Rs(i)-1);
    ...
    Xd(:,i,1) = 400*(PId(i)-1);
%     Xd(:,i,2) = 100*(Yd(i)-1);
    Xd(:,i,2) = 100*(cVARPHI/2*(PId(i)/cPItarg(i)-1)^2*Yd(i));
    Xd(:,i,3) = 100*(Cd(i)-1);
    Xd(:,i,4) = 400*(Rd(i)-1);
...
% print(fig(1),'-depsc','sunspot_only_IRFs_2by2.eps');
print(fig(1),'-depsc','sunspot_only_IRFs_2by2_pac.eps');
% print(fig(1),'-depsc',strcat(savedir,'sunspot_only_IRFs_2by2.eps'));
print(fig(1),'-depsc',strcat(savedir,'sunspot_only_IRFs_2by2_pac.eps'));