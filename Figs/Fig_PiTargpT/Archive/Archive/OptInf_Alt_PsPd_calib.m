%--------------------------------------------------------------------------
%File Name: OptInf_Alt_PsPd_calib.m
%Author: Philip Coyle
%Date Created: 01/17/2018
% cd /mq/philipprojects/RA_Work/Taisuke_Nakata/Zero_Lower_Bound/DeflationaryRegime/draft/Figs/Fig_6
% OptInf_Alt_PsPd_calib.m
%--------------------------------------------------------------------------
...
cTHETA      = 11;
cTAU        = 1/cTHETA;
cVARPHI     = 1300; % For an approx 200 bps decline in inflation 
...
cIOTA       = 1;
% cALPHA      = 0.7945; 
cALPHA      = 0.95; % For an approx 200 bps decline in inflation 
...
p_s         = 0.995;%(0.99:0.001:1); 
p_d         = 0.975;%[0.925,0.95,.975,.985,.995]; 
cDELz       = 1;
c           = 0.9/100;
cDELc       = 1 + c;
p_z         = 0.995;  
...
if cVARPHI == 1250 && cALPHA == 0.876
    display('Running 200 BPS case')
elseif cVARPHI == 2550 && cALPHA == 0.915
    display('Running 100 BPS case')
end
...
%     if cVARPHI == 1250 && cALPHA == 0.876
%         save('alt_PsPd_data_200.mat','Csz','PIsz','Ysz','Nsz','Wsz','Rsz','Vsz','Csc','PIsc','Ysc','Nsc','Wsc','Rsc','Rsc_check','Vsc','Cdz','PIdz','Ydz','Ndz','Wdz','Rdz','Vdz','Cdc','PIdc','Ydc','Ndc','Wdc','Rdc','Rdc_check','Vdc');
%     elseif cVARPHI == 2550 && cALPHA == 0.915
%         save('alt_PsPd_data_100.mat','Csz','PIsz','Ysz','Nsz','Wsz','Rsz','Vsz','Csc','PIsc','Ysc','Nsc','Wsc','Rsc','Rsc_check','Vsc','Cdz','PIdz','Ydz','Ndz','Wdz','Rdz','Vdz','Cdc','PIdc','Ydc','Ndc','Wdc','Rdc','Rdc_check','Vdc');
%     end
else
    if cVARPHI == 1250 && cALPHA == 0.876
        load('alt_PsPd_data_200.mat')
    elseif cVARPHI == 2550 && cALPHA == 0.915
        load('alt_PsPd_data_100.mat')
    end
end
%% Main Code
gamma = nan(4,16^2);
...
colors = {'k','b','r','g','y','o'};
pd_string = {'p_d = 0.925','p_d = 0.95','p_d = 0.975','p_d = 0.985','p_d = 0.995'};
...
for i = 1:length(p_d)
    ...
    xlabel('p_s','FontSize',15)
    ylabel('Optimal Inflation','FontSize',15)
    set(gca,'XLim',[0.99 1],'Ylim',[-1 2],'FontSize',15)
    
end
% L = legend([h(1) h(2) h(3) h(4) h(5) h(6)],pd_string{1},pd_string{2},pd_string{3},pd_string{4},pd_string{5},pd_string{6},'Location', 'NorthWest');
L = legend([h(1) h(2) h(3) h(4) h(5)],pd_string{1},pd_string{2},pd_string{3},pd_string{4},pd_string{5},'Location', 'NorthWest');1


set(fig(1),'PaperOrientation','Landscape');
set(fig(1),'PaperPosition',[0 0 11 8.5]);
if cVARPHI == 1250 && cALPHA == 0.876
    print(fig(1),'-depsc','opt_inf_vary_ps_pd_200bps.eps');
elseif cVARPHI == 2550 && cALPHA == 0.915
    print(fig(1),'-depsc','opt_inf_vary_ps_pd_100bps.eps');
end
% print(fig(1),'-depsc','opt_inf_vary_ps_pd_chic05_chin2.eps');
