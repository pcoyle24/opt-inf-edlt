function residuals_markov_sss = demand_solve_alpha(x_guess_markov_sss,j,cALPHA)

global cBET cCHIc cCHIn cTHETA cTAU cVARPHI cPHIpi cPHIy cRzlb cIOTA cPItarg cDELz cDELc p_z p_c
...
% Rsc  = (cPItarg(j)/(cBET))*((PIsc/cPItarg(j))^cPHIpi)*((Ysc/Ysc)^cPHIy);
if Rsc < 1
    Rsc = cRzlb;
end
...
residuals_markov_sss(2) = Ysz/Csz^(cCHIc)*(cVARPHI*(PIsz/((cPItarg(j)^cIOTA*PIsz^(1-cIOTA))^cALPHA) - 1)*PIsz/((cPItarg(j)^cIOTA*PIsz^(1-cIOTA))^cALPHA) - (1 - cTHETA) - cTHETA*(1-cTAU)*Wsz) - (cBET*cDELz*cVARPHI*(p_z*((Ysz/Csz^(cCHIc))*(PIsz/((cPItarg(j)^cIOTA*PIsz^(1-cIOTA))^cALPHA) - 1)*PIsz/((cPItarg(j)^cIOTA*PIsz^(1-cIOTA))^cALPHA)) + (1-p_z)*((Ysc/Csc^(cCHIc))*(PIsc/((cPItarg(j)^cIOTA*PIsc^(1-cIOTA))^cALPHA) - 1)*PIsc/((cPItarg(j)^cIOTA*PIsc^(1-cIOTA))^cALPHA))));
residuals_markov_sss(3) = Ysz - Csz - cVARPHI/2*(PIsz/((cPItarg(j)^cIOTA*PIsz^(1-cIOTA))^cALPHA) - 1)^2*Ysz;
...
residuals_markov_sss(7) = Ysc/Csc^(cCHIc)*(cVARPHI*(PIsc/((cPItarg(j)^cIOTA*PIsc^(1-cIOTA))^cALPHA) - 1)*PIsc/((cPItarg(j)^cIOTA*PIsc^(1-cIOTA))^cALPHA) - (1 - cTHETA) - cTHETA*(1-cTAU)*Wsc) - (cBET*cDELc*cVARPHI*(p_c*((Ysc/Csc^(cCHIc))*(PIsc/((cPItarg(j)^cIOTA*PIsc^(1-cIOTA))^cALPHA) - 1)*PIsc/((cPItarg(j)^cIOTA*PIsc^(1-cIOTA))^cALPHA)) + (1-p_c)*((Ysz/Csz^(cCHIc))*(PIsz/((cPItarg(j)^cIOTA*PIsz^(1-cIOTA))^cALPHA) - 1)*PIsz/((cPItarg(j)^cIOTA*PIsz^(1-cIOTA))^cALPHA))));
residuals_markov_sss(8) = Ysc - Csc - cVARPHI/2*(PIsc/((cPItarg(j)^cIOTA*PIsc^(1-cIOTA))^cALPHA) - 1)^2*Ysc;
