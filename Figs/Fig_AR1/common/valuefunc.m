...
addpath('/mq/philipprojects/RA_Work/Taisuke_Nakata/Zero_Lower_Bound/DeflationaryRegime/OptInf/Codes/AR1/1d/calib/Fortran')

%% Load in Data (Demand shock only)
pitarg_col = 2; 
val_col = 3; 

data = dlmread('exp_val_opt_inf_10bps_1001.dat');

pi_targ  = data(:,pitarg_col);
value_d  = data(:,val_col);

max_val_inx = find(value_d == max(value_d));
opt_inf_d = pi_targ(max_val_inx);
...
set(gca,'XLim',[pi_targ(1) pi_targ(end)],'YLim',[min(value_ds)-0.2 max(value_ds)+ 0.2],'FontSize',16)
...
print(fig(1),'-depsc','Valuefunc_sunspot_del101.eps');
