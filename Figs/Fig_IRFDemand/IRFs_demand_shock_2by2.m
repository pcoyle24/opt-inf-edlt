%--------------------------------------------------------------------------
%File Name: IRFs_demand_shock_2by2.m
%Author: Philip Coyle
%Date Created: 02/02/2018
% cd /mq/philipprojects/RA_Work/Taisuke_Nakata/Zero_Lower_Bound/DeflationaryRegime/draft/Figs/Fig_3
% IRFs_demand_shock_2by2
%--------------------------------------------------------------------------
...
%     Xsz(:,i,2) = 100*(Ysz(i)-1);
    Xsz(:,i,2) = 100*(cVARPHI/2*(PIsz(i)/cPItarg(i)-1)^2*Ysz(i));
    Xsz(:,i,3) = 100*(Csz(i)-1);
    Xsz(:,i,4) = 400*(Rsz(i)-1);
...
%     Xsc(:,i,2) = 100*(Ysc(i)-1);
    Xsc(:,i,2) = 100*(cVARPHI/2*(PIsc(i)/cPItarg(i)-1)^2*Ysc(i));
    Xsc(:,i,3) = 100*(Csc(i)-1);
    Xsc(:,i,4) = 400*(Rsc(i)-1);
...
% header = {'Inflation','Output','Consumption','Policy Rate'};
header = {'Inflation','Price Adj. Cost','Consumption','Policy Rate'};
...
L = legend([h(1) h(2)],pi_targ_string{1},pi_targ_string{2},'Location', 'South');
...
% print(fig(1),'-depsc','demand_only_IRFs_2by2.eps');
print(fig(1),'-depsc','demand_only_IRFs_2by2_pac.eps');
% print(fig(1),'-depsc',strcat(savedir,'demand_only_IRFs_2by2.eps'));
print(fig(1),'-depsc',strcat(savedir,'demand_only_IRFs_2by2_pac.eps'));
